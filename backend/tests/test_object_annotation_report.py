...
SCRIPT = Path(__file__).parents[2] / "scripts" / "diagnostics" / "generate-object-annotation-report.py"
...
def test_report_contains_review_state_validation_and_hashes(tmp_path: Path) -> None:
    ...
    (sampling.parent / "report.json").write_text(json.dumps({"source_sha256": "a" * 64}), encoding="utf-8")
    ...
    payload = {
        ...
        "sources": [{
            "source": "sample_1.mp4",
            "source_sha256": "a" * 64,
            "frames": [{"filename": "frame.jpg", "timestamp_sec": 1.0, "objects": [], "out_of_taxonomy": []}],
        }],
    }
    ...
    report, markdown = reporter.generate_report(merged, tmp_path / "sampling", [reviewer, reviewer])
