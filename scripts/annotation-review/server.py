...
class ReviewServer(ThreadingHTTPServer):
    def __init__(self, address: tuple[str, int], handler: type[BaseHTTPRequestHandler], root: Path, annotations: Path, sampling: Path):
        super().__init__(address, handler)
        self.review_root = root
        self.annotation_path = annotations.resolve()
        self.sampling_root = sampling.resolve()
        self.bundle_root = self.annotation_path.parent / "agent-bundles"
...
class ReviewHandler(BaseHTTPRequestHandler):
    ...
    def _generate_bundles(self) -> None:
        ...
        subprocess.run(
            [sys.executable, str(exporter), str(self.server.annotation_path), str(self.server.sampling_root), str(self.server.bundle_root)],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def _import_agent(self, content: object) -> dict[str, object]:
        ...
        source = next((item for item in original["sources"] if item.get("source") == source_name), None)
        ...
        if not isinstance(incoming, list) or {frame.get("filename") for frame in incoming} != set(expected):
            raise ValueError("agent response must contain every manifest frame exactly once")
        for frame in incoming:
            ...
            if not isinstance(frame.get("objects"), list) or not isinstance(frame.get("out_of_taxonomy"), list):
                raise ValueError(f"invalid annotation lists for {frame['filename']}")
        for frame in source["frames"]:
            replacement = next(item for item in incoming if item["filename"] == frame["filename"])
    ...
    def do_HEAD(self) -> None:  # noqa: N802
        ...
        if parsed.path.startswith("/api/bundles/"):
            ...
            bundle_root = self.server.bundle_root.resolve()
            bundle_path = (bundle_root / filename).resolve()
            if bundle_path.is_relative_to(bundle_root) and bundle_path.is_file() and bundle_path.suffix == ".zip":
                self.send_response(HTTPStatus.OK)
    ...
    def _send_frame(self, query: dict[str, list[str]]) -> None:
        ...
        if filename not in frame_names:
            self._send_json({"error": "frame is not in annotation manifest"}, HTTPStatus.NOT_FOUND)
            return
        frames_root = self.server.sampling_root / Path(source).stem / "hybrid" / "frames"
        ...
        if not image_path.is_relative_to(frames_root.resolve()):
            ...
            return
        self._send_file(image_path, mimetypes.guess_type(image_path.name)[0] or "image/jpeg")
    ...
    def _validate_identity(self, payload: object) -> None:
        ...
        original_identity = [
            (source.get("source"), source.get("source_sha256"), [frame.get("filename") for frame in source.get("frames", [])])
        ...
        if new_identity != original_identity:
            raise ValueError("source checksums and manifest frame identity cannot change")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the local object annotation review UI")
    ...
    server = ReviewServer(
        (args.host, args.port), ReviewHandler, review_root, args.annotations, args.sampling_root
    )
